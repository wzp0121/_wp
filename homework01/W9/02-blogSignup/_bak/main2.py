...
from fastapi_sessions.session_verifiers import get_session_cookie
...
# 初始化 InMemoryBackend，並將其設為 session 存儲後端
in_memory_backend = InMemoryBackend()

# 添加 session middleware，並指定使用 InMemoryBackend
app.add_middleware(SessionMiddleware, backend=in_memory_backend, secret_key="your-secret-key", cookie_name="session_id")
...
@app.post("/login")
async def login(request: Request, username: str = Form(...), password: str = Form(...), session: Session = Depends(get_session_cookie)):
...
# 登出
@app.get("/logout")
async def logout(request: Request, session: Session = Depends(get_session_cookie)):
    session.clear()
    return RedirectResponse(url="/", status_code=303)

# 文章頁面
@app.get("/post/new", response_class=HTMLResponse)
async def add_post(request: Request, session: Session = Depends(get_session_cookie)):
...
@app.post("/post")
async def create_post(request: Request, title: str = Form(...), body: str = Form(...), session: Session = Depends(get_session_cookie)):
    if "user" not in session:
